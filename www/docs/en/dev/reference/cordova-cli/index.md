...
| Option | Description |
|--------|-------------|
| --template |  Use a custom template located locally, in NPM, or GitHub. <br/> works with a project, a template (specially formatted project), or a www folder as a template source|
| --copy-from\|--src | _Will be deprecated_ <br/> alias for --template|
...
- Create a Cordova project from a template. This can be useful if you have a custom build process or existing web assets that you want to use in your Cordova app:

        cordova create hello com.ex.hello Hello --template <npm-package-name | <git-remote-url | local-path>
